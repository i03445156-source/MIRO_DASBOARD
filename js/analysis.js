...
//  메인 분석 실행
// ══════════════════════════════════════════════════════════════════════
export async function runAnalysis(stockName, customStart = null, customEnd = null) {
  ...
  const ohlc = await fetchOHLC(ticker, startStr, customEnd || null);
  ...
  const closes = customStart ? allCloses : allCloses.slice(-200);
  const dates  = customStart ? allDates  : allDates.slice(-200);
  ...
  const ma200 = customStart ? ma200full : ma200full.slice(-200);
