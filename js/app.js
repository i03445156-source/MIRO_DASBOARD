...
function switchTab(tabId) {
  ...
  if (tabId === 'macro' && !macroLoaded) {
    macroLoaded = true;
    const days = parseInt(document.getElementById('macro-period')?.value || '90');
    loadMacroTab(days);
  }
...
function populateSelects() {
  ...
  // 리스크 탭 기본 시작일 (1년 전)
  const riskStartEl = document.getElementById('risk-start-date');
  if (riskStartEl && !riskStartEl.value) {
    const d = new Date(); d.setFullYear(d.getFullYear() - 1);
    riskStartEl.value = d.toISOString().split('T')[0];
  }

  // 포트폴리오 탭 기본 시작일 (1년 전)
  const portStartEl = document.getElementById('port-start-date');
  if (portStartEl && !portStartEl.value) {
    const d = new Date(); d.setFullYear(d.getFullYear() - 1);
    portStartEl.value = d.toISOString().split('T')[0];
  }

  // 종목분석 탭 기본 날짜
  const anaStartEl = document.getElementById('ana-start-date');
  const anaEndEl   = document.getElementById('ana-end-date');
  if (anaEndEl && !anaEndEl.value) anaEndEl.value = new Date().toISOString().split('T')[0];
  if (anaStartEl && !anaStartEl.value) {
    const d = new Date(); d.setDate(d.getDate() - 420);
    anaStartEl.value = d.toISOString().split('T')[0];
  }
}
...
function initReturnTab() {
  document.getElementById('ret-load-btn').addEventListener('click', async () => {
    ...
    const startDate = document.getElementById('ret-start-date')?.value || null;
    showLoading('수익률 로딩 중...');
    try { await loadReturns(member, startDate); }
...
function initRiskTab() {
  document.getElementById('risk-run-btn').addEventListener('click', async () => {
    ...
    const startDate = document.getElementById('risk-start-date')?.value || null;
    showLoading('리스크 분석 중...');
    try { await runRiskAnalysis(sel, inv, conf, startDate); }
    catch (e) { console.error(e); }
    finally { hideLoading(); }
  });
}
...
function initPortfolioTab() {
  document.getElementById('port-run-btn').addEventListener('click', async () => {
    ...
    const startDate = document.getElementById('port-start-date')?.value || null;
    showLoading('포트폴리오 최적화 중...');
    try { await runPortfolioOptimization(sel, rf, nSim, startDate); }
    catch (e) { console.error(e); }
    finally { hideLoading(); }
  });
}
...
function initAnalysisTab() {
  document.getElementById('ana-run-btn').addEventListener('click', async () => {
    ...
    const customStart = document.getElementById('ana-start-date')?.value || null;
    const customEnd   = document.getElementById('ana-end-date')?.value   || null;
    showLoading(`${stock} 분석 중...`);
    try { await runAnalysis(stock, customStart, customEnd); }
...
function initMacroTab() {
  const reloadBtn = document.getElementById('macro-reload-btn');
  if (!reloadBtn) return;
  reloadBtn.addEventListener('click', async () => {
    ...
    try { await loadMacroTab(days); }
    catch (e) { console.error(e); }
    finally { reloadBtn.textContent = '↺ 새로고침'; reloadBtn.disabled = false; }
  });
}
